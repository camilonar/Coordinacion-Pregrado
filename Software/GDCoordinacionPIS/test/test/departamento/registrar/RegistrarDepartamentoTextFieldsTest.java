 ...
 */
package test.departamento.registrar;

import test.usuario.registrar.*;
...
/**
 * Pruebas de los campos de texto del formulario de registro de departamento
 * @author Camilo
 */
@RunWith(Parameterized.class)
public class RegistrarDepartamentoTextFieldsTest extends AbstractDepartamentoTest{
    ...
    public static Collection<Object[]> data() {
        return Arrays.asList(new Object[][] {    
            ...
            { RegistrarDepartamentoNames.idNombre,"SistemasSistemasSistemasSistemasSistemasSistemasSistemasSistemasSistemasSistemasSistemasSistemasERTQZ",RegistrarDepartamentoNames.idMsgNombre,false},
            { RegistrarDepartamentoNames.idNombre,"Automática1",RegistrarDepartamentoNames.idMsgNombre,false},
            { RegistrarDepartamentoNames.idNombre,"F#$_°()",RegistrarDepartamentoNames.idMsgNombre,false},
            { RegistrarDepartamentoNames.idNombre,"12345",RegistrarDepartamentoNames.idMsgNombre,false},
            { RegistrarDepartamentoNames.idNombre,"",RegistrarDepartamentoNames.idMsgNombre,false},
            { RegistrarDepartamentoNames.idNombre,"Contables",RegistrarDepartamentoNames.idMsgNombre,false}
        });
    }
    ...
    public void testRegistrar() {
        goToRegistrar();
        JWebUnit.setWorkingForm(RegistrarDepartamentoNames.idFormulario);
        JWebUnit.assertFormPresent(RegistrarDepartamentoNames.idFormulario);
        JWebUnit.setTextField(idCampo, contenido);
        JWebUnit.assertSubmitButtonPresent(RegistrarDepartamentoNames.idRegistrarBtn);
        JWebUnit.submit(RegistrarDepartamentoNames.idRegistrarBtn);
        JWebUnit.assertElementPresent(idMensaje);
        try {
            Thread.sleep(1500);
        } catch (InterruptedException ex) {
            Logger.getLogger(RegistrarDepartamentoTextFieldsTest.class.getName()).log(Level.SEVERE, null, ex);
        }
        if (isCorrect){
